...
async function loadCircle(){
  ...
  if(!r.ok)throw new Error("could not load data/circle.json ("+r.status+"). "
    +"If you opened this file directly, serve it instead: python tools/serve.py");
  return r.json();
}
(async function boot(){
let CIRCLE;
try{ CIRCLE=await loadCircle(); }
catch(err){
...
function sortKey(d){if(!d||d.y==null)return Infinity;return d.y+((d.m??6.5)-1)/12+((d.d??15)/372);}
...
function fmtDate(d){
  ...
  if(t&&t.y!=null&&!(t.y===d.y&&t.m===d.m&&t.d===d.d)){
    if(t.y===d.y){
      if(d.m&&t.m===d.m&&d.d&&t.d)return `${c}${d.d}–${t.d} ${MONTHS[d.m-1]} ${d.y}`;
      if(d.m&&t.m&&!d.d&&!t.d)return `${c}${MONTHS[d.m-1]}–${MONTHS[t.m-1]} ${d.y}`;
...
function yearOf(d){return d&&d.y!=null?String(d.y):"?";}
...
const CERT_LABEL={"self-reported":"Self-Reported","second-hand":"Second-Hand","uncorroborated":"Uncorroborated",
  "married":"Married","attraction-expressed":"Attraction expressed",[NO_EDGE]:"No connection"};
const OUTCOME_LABEL={declined:"declined",unknown:"outcome unknown",unreciprocated:"not reciprocated"};
const DEFAULT_OFF=new Set([]);   /* attraction-expressed switched ON by default 2026-08-04 - the
   layout now separates those clusters cleanly enough to show them without crowding */
const VER_META={"verified-exact":["✓ Verified","v-ok"],"verified-elision":["✓ Verified","v-ok"],
  "needs-fix":["! Needs fix","v-warn"],"rejected":["✗ Rejected","v-bad"],
  "unverified":["⧖ Pending verification","v-pend"]};
const HOW_LABEL={"pdf-at-page":"read in the repo PDF","repo-file":"repo transcript","archive-org":"read on archive.org","web":"web source","unverified":""};
const GROUP_LABEL={core:"Wilde",family:"Spouse",society:"Society",aesthete:"Aesthetes",
  trials:"1895 trials",chaeronea:"Chaeronea",later:"Later circle",
  liaisons:"Outside the circle",beyond:"Beyond Europe"};

/* ============ data prep ============ */
const P=new Map(CIRCLE.people.map(p=>[p.id,p]));
const RELS=CIRCLE.relationships.filter(r=>r.people.every(id=>P.has(id)));
const byPerson=new Map();
for(const r of RELS)for(const id of r.people){if(!byPerson.has(id))byPerson.set(id,[]);byPerson.get(id).push(r);}
...
function shortLabelRaw(p){
  ...
  const clash=CIRCLE.people.some(o=>o.id!==p.id&&(o.sort_name||o.name).split(",")[0].trim()===sur);
  if(!clash)return sur;
  ...
  const first=((p.name||"").split(/\s+/)
    .find(w=>!/^(sir|lord|lady|dame|rev|revd|mr|mrs|miss|ms|dr|capt|captain|admiral|col|colonel|gen|general|hon)\.?$/i.test(w)))||"";
  return `${sur} ${first?first[0]+".":""}`.trim();
}
function partnerOf(r,id){return r.people[0]===id?r.people[1]:r.people[0];}
function verCounts(){
  let ok=0,pend=0,total=0,pointers=0;
  for(const r of RELS)for(const q of r.sources||[]){
    total++;
    if((q.quote||"")==="")pointers++;
    if(q.verification==="verified-exact"||q.verification==="verified-elision")ok++;
    else pend++;
  }
...
const W=1200,H=800;
const SIM={K_REP:36000,K_SPRING:0.035,REST:150,K_CENTER:0.02,DAMP:0.85,MAX_V:14,
  ALPHA0:1,ALPHA_DECAY:0.004,ALPHA_MIN:0.02,PAD:14};
const CLEAR=13, MAX_SHOVE=7;   // clearance from unconnected lines; per-tick displacement cap
...
const LABEL_BELOW=15, LABEL_H=13, LABEL_PAD=3, LABEL_CH=3.0;
...
function boxPush(a,b,pad,apply){
  ...
  const ox=(a.hw+b.hw+pad)-Math.abs(dx), oyv=(a.hh+b.hh+pad)-Math.abs(dy);
  if(ox<=0||oyv<=0)return 0;
  if(ox<oyv){const s=(dx<0?-1:1)*ox/2; apply(-s,0,s,0); return ox;}
  const s=(dy<0?-1:1)*oyv/2; apply(0,-s,0,s); return oyv;
}
...
const nodes=CIRCLE.people.map(p=>({id:p.id,p,r:16+3*Math.sqrt(degree(p.id)),x:0,y:0,vx:0,vy:0,
  pinned:false,hx:(p.layout&&p.layout.x)??null,hy:(p.layout&&p.layout.y)??null}));
nodes.sort((a,b)=>degree(b.id)-degree(a.id));
nodes.forEach((n,i)=>{
  const ang=i*2.39996, rad=30+16*Math.sqrt(i)*4;
  n.x=W/2+Math.cos(ang)*rad+rand()*8; n.y=H/2+Math.sin(ang)*rad+rand()*8;   // circular; see below
...
const edges=RELS.map(r=>({r,a:N.get(r.people[0]),b:N.get(r.people[1])}));
...
for(const e of edges){
  const leaf=(byPerson.get(e.a.id)||[]).length===1||(byPerson.get(e.b.id)||[]).length===1;
...
const compOf=new Map(), compHome=new Map(), compSoft=new Set();
let mainComp=0;
{
    ...
    for(const n of nodes){
      ...
      while(stack.length){
        ...
        for(const r of byPerson.get(id)||[]){
          ...
          if(!map.has(o)){map.set(o,c);stack.push(o);}
        }
      }
      c++;
    }
  ...
  const SLOTS=[[185,175],[185,625],[1015,175],[1015,625],[600,100],[600,705],[390,120],[810,120]];
  const memberOf=new Map();
  for(const n of nodes){if(!memberOf.has(compOf.get(n.id)))memberOf.set(compOf.get(n.id),n);}
  const leadOnly=[];                              // singletons held only by a hidden link
  let s=0,solo=0;
  ranked.forEach(([ci,sz],idx)=>{
    if(idx===0){compHome.set(ci,[W/2,H/2]);return;}
    if(sz>1&&s<SLOTS.length){compHome.set(ci,SLOTS[s++]);return;}
    if(degree(memberOf.get(ci).id)){leadOnly.push(ci);compHome.set(ci,null);return;}
    ...
    const side=(solo%2)?W-70:70, row=Math.floor(solo/2);
    compHome.set(ci,[side,285+row*115]); solo++;
  });
  ...
  for(const ci of leadOnly){
    ...
    const anchor=[...full].filter(([id,f])=>f===full.get(n.id)&&id!==n.id)
                          .map(([id])=>compHome.get(compOf.get(id))).find(Boolean);
    if(anchor){compHome.set(ci,anchor);compSoft.add(ci);}
  }
  for(const n of nodes){              // start satellites at home so they don't migrate across
    const home=compHome.get(compOf.get(n.id));
    if(home&&compOf.get(n.id)!==mainComp){n.x=home[0]+(rand()-0.5)*70;n.y=home[1]+(rand()-0.5)*70;}
  }
     ...
     rather than replacing it. */
  {
    ...
    const kids=new Map(), depth=new Map([[hub.id,0]]), order=[hub.id];
    for(let i=0;i<order.length;i++){
      const id=order[i]; kids.set(id,[]);
      for(const r of byPerson.get(id)||[]){
        const o=partnerOf(r,id);
        if(!inMain(o)||depth.has(o))continue;      // chords land back on a placed node: skip
        depth.set(o,depth.get(id)+1); kids.get(id).push(o); order.push(o);
      }
    }
    const leaves=new Map();                        // angular weight = leaf count of the subtree
    for(let i=order.length-1;i>=0;i--){
      const id=order[i], ch=kids.get(id)||[];
      leaves.set(id, ch.length?ch.reduce((s,c)=>s+leaves.get(c),0):1);
    }
        ...
        if(ch.length>1&&angleOf.size>1){
          const bary=c=>{
            ...
            for(const r of byPerson.get(c)||[]){
              ...
              if(o!==id&&angleOf.has(o)&&!(kids.get(c)||[]).includes(o))xs.push(angleOf.get(o));
            }
            return xs.length?xs.reduce((a,b)=>a+b,0)/xs.length:null;
          };
          const keyed=ch.map(c=>[c,bary(c)]);
          if(keyed.some(k=>k[1]!=null))
            keyed.sort((A,B)=>(A[1]??(a0+a1)/2)-(B[1]??(a0+a1)/2));
          ch.length=0; for(const [c] of keyed)ch.push(c);
          kids.set(id,ch);
        }
        let a=a0;
        for(const c of ch){
    ...
    layout(); layout();                    // second pass: the barycentres are better informed
    for(const [id,mid] of angleOf){
      const d=depth.get(id), rad=RING[Math.min(d,RING.length-1)]+(d>=RING.length?(d-RING.length+1)*70:0);
      const n=N.get(id);
      ...
      n.x=W/2+Math.cos(mid)*rad; n.y=H/2+Math.sin(mid)*rad;
    }
    for(const n of nodes)if(compOf.get(n.id)===mainComp){n.sx=n.x;n.sy=n.y;n.seedDepth=depth.get(n.id);}
    ...
    for(const n of nodes)if(n.hx!=null){n.x=n.hx;n.y=n.hy;}   // hints still win
  }
}
let alpha=SIM.ALPHA0,running=false,raf=0;
...
function tick(){
  ...
  for(let i=0;i<nodes.length;i++)for(let j=i+1;j<nodes.length;j++){
    ...
    let d=Math.sqrt(d2)||0.01; if(d<8){d=8;d2=64;}
    // push harder between people in different components so territories stay distinct
    const f=SIM.K_REP/d2*(compOf.get(a.id)!==compOf.get(b.id)?1.8:1), fx=f*dx/d, fy=f*dy/d;
    a.fx+=fx;a.fy+=fy;b.fx-=fx;b.fy-=fy;
  }
  for(const e of edges){
    const dx=e.b.x-e.a.x,dy=e.b.y-e.a.y,d=Math.sqrt(dx*dx+dy*dy)||0.01;
    const f=e.k*(d-e.rest), fx=f*dx/d, fy=f*dy/d;
    e.a.fx+=fx;e.a.fy+=fy;e.b.fx-=fx;e.b.fy-=fy;
  }
  for(const n of nodes){
    const home=compHome.get(compOf.get(n.id));
    if(home&&compOf.get(n.id)!==mainComp){          // satellite: hold its own territory
      ...
      const k=compSoft.has(compOf.get(n.id))?0.010:0.055;
      n.fx+=(home[0]-n.x)*k; n.fy+=(home[1]-n.y)*k;
    }else{
      const pull=SIM.K_CENTER*Math.sqrt(degree(n.id)+1);
      n.fx+=(W/2-n.x)*pull*0.05; n.fy+=(H/2-n.y)*pull*0.05;
    }
  }
  for(const n of nodes){
    if(n.pinned)continue;
    n.vx=(n.vx+n.fx*alpha)*SIM.DAMP; n.vy=(n.vy+n.fy*alpha)*SIM.DAMP;
    const v=Math.hypot(n.vx,n.vy);
    if(v>SIM.MAX_V){n.vx*=SIM.MAX_V/v;n.vy*=SIM.MAX_V/v;}
    n.x+=n.vx;n.y+=n.vy;
  }
  for(let pass=0;pass<2;pass++)for(let i=0;i<nodes.length;i++)for(let j=i+1;j<nodes.length;j++){
    const a=nodes[i],b=nodes[j];
    boxPush(a,b,SIM.PAD*0.5,(axd,ayd,bxd,byd)=>{
      if(!a.pinned){a.x+=axd;a.y+=ayd;}
      if(!b.pinned){b.x+=bxd;b.y+=byd;}
    });
  }
  for(const n of nodes){          // placement hints pull far harder than the centring force
    if(n.hx==null||n.pinned)continue;
    n.vx+=(n.hx-n.x)*0.10; n.vy+=(n.hy-n.y)*0.10;
  }
  if(!HUB.pinned&&HUB.hx==null){  // hold the hub in the middle; everything else arranges around it
    HUB.vx+=(W/2-HUB.x)*0.12; HUB.vy+=(H/2-HUB.y)*0.12;
  }
  ...
  for(const n of nodes){
    ...
    for(const e of edges){
      ...
      const ox=n.x-px,oy=n.y-py,d=Math.hypot(ox,oy)||0.01;
      const min=n.r+CLEAR;
      if(d<min){ax+=ox/d*(min-d);ay+=oy/d*(min-d);}
    }
    if(!ax&&!ay)continue;
    let sx=ax*0.5,sy=ay*0.5;                  // half the correction; the rest next tick
    const mag=Math.hypot(sx,sy);
    if(mag>MAX_SHOVE){sx*=MAX_SHOVE/mag;sy*=MAX_SHOVE/mag;}
...
function loop(){
  if(alpha<=SIM.ALPHA_MIN){running=false;return;}
  tick();render();raf=requestAnimationFrame(loop);
}
function reheat(a){
  alpha=Math.max(alpha,a);
  if(!running&&!reduceMotion){running=true;raf=requestAnimationFrame(loop);}
  if(reduceMotion){for(let i=0;i<300;i++)tick();render();}
}

/* ============ svg build ============ */
const svg=$("#web"),gE=$("#gEdges"),gN=$("#gNodes"),tip=$("#tip");
const NS="http://www.w3.org/2000/svg";
function mk(t,attrs,parent){const el=document.createElementNS(NS,t);for(const k in attrs)el.setAttribute(k,attrs[k]);if(parent)parent.appendChild(el);return el;}
for(const e of edges){
  const g=mk("g",{class:`edge cert-${e.r.certainty}${e.r.outcome?" out-"+e.r.outcome:""}`,"data-id":e.r.id},gE);
  e.under=mk("path",{class:"e-under"},g);
  e.main=mk("path",{class:"e-main"},g);
  e.hit=mk("path",{class:"e-hit"},g);
  e.g=g;
  g.addEventListener("click",ev=>{ev.stopPropagation();location.hash=`#/r/${e.r.id}`;});
...
for(const n of nodes){
  const g=mk("g",{class:`node g-${n.p.group}`,"data-id":n.id,tabindex:"0",role:"button",
    "aria-label":`${n.p.name}, open profile`},gN);
  const s=n.r*1.78;
  if(n.p.gender==="m"){
    mk("rect",{class:"shape",x:-s/2,y:-s/2,width:s,height:s,rx:2},g);
  }else{
  ...
  if(por){
    const cid="clip-"+n.id, cp=mk("clipPath",{id:cid},defs);
    if(n.p.gender==="m")mk("rect",{x:-s/2,y:-s/2,width:s,height:s,rx:2},cp);
    else mk("circle",{r:n.r},cp);
    const im=mk("image",{class:"portrait","clip-path":`url(#${cid})`,
      x:-s/2,y:-s/2,width:s,height:s,preserveAspectRatio:"xMidYMin slice"},g);
  ...
  n.t=t; n.ly=n.r+LABEL_BELOW;
  n.g=g;
  g.addEventListener("keydown",ev=>{if(ev.key==="Enter"||ev.key===" "){ev.preventDefault();location.hash=`#/p/${n.id}`;}});
}
fitLabelBoxes();
...
function trimPastLabel(n,dx,dy,base){return base;}
function render(){
  for(const e of edges){
    ...
    const src=(arrow&&e.r.direction===e.b.id)?e.b:e.a, tgt=(src===e.a)?e.b:e.a;
    ...
    if(len<=ts+tt+6){ts=src.r;tt=tgt.r+(arrow?11:0);}
    ...
    let dm;
    if(len<=ts+tt+2){        // nodes overlapping: nothing sane to draw between them
      dm="";
    }else{
      dm=`M${(src.x+ux*ts).toFixed(1)},${(src.y+uy*ts).toFixed(1)}`
  ...
  for(const n of nodes)n.g.setAttribute("transform",`translate(${n.x.toFixed(1)},${n.y.toFixed(1)})`);
}
...
function zoomAbout(scale,cx,cy){
  ...
  const nw=W*scale, nh=nw/paneAspect();
  vb.x=wx-(wx-vb.x)*nw/vb.w; vb.y=wy-(wy-vb.y)*nh/vb.h;
  vb.w=nw; vb.h=nh;
  applyVB();
}
...
function beginPinch(){
  ...
  svg._pinch={d:Math.hypot(pts[0][0]-pts[1][0],pts[0][1]-pts[1][1]),w:[wx,wy],vb:{...vb}};
}
svg.addEventListener("contextmenu",ev=>ev.preventDefault());   // right-drag must not open the menu
svg.addEventListener("auxclick",ev=>{if(PAN_BUTTON(ev))ev.preventDefault();});
svg.addEventListener("mousedown",ev=>{if(ev.button===1)ev.preventDefault();});  // no autoscroll
svg.addEventListener("pointerdown",ev=>{
  pointers.set(ev.pointerId,[ev.clientX,ev.clientY]);
  if(pointers.size>1){drag=null;beginPinch();return;}
  ...
  if(nodeEl){
    ...
    drag={node:n,dx:n.x-wx,dy:n.y-wy,sx:ev.clientX,sy:ev.clientY,t0:performance.now(),moved:0};
    svg.setPointerCapture(ev.pointerId);
  }else if(panning||!ev.target.closest(".edge")){
    drag={pan:true,btn:ev.button,px:ev.clientX,py:ev.clientY,
          sx:ev.clientX,sy:ev.clientY,t0:performance.now(),moved:0};
...
svg.addEventListener("pointermove",ev=>{
  ...
  if(pointers.size===2){
    ...
    const p=svg._pinch;
    if(!p)return;
    const scale=Math.max(0.35,Math.min(ZOOM_OUT_MAX,(p.vb.w/W)*(p.d/Math.max(d,1))));
    const r=svg.getBoundingClientRect();
    vb.w=W*scale; vb.h=vb.w/paneAspect();
  ...
  if(drag.node){
    ...
    if(drag.moved>3){drag.node.pinned=true;drag.node.g.classList.add("pinned");reheat(0.25);}
    render();
  }else if(drag.pan){
    const r=svg.getBoundingClientRect();
...
function endPointer(ev){
  ...
  if(drag.node&&quick)location.hash=`#/p/${drag.node.id}`;
  /* A quick tap on empty ground clears the selection. Only the LEFT button does that: a stray
     right- or middle-click is a pan that happened not to move, and having it deselect the person
     whose sources you are reading would be its own small betrayal. */
  if(drag.pan&&quick&&!drag.btn){if(location.hash&&!$("#about").classList.contains("open"))location.hash="";}
  drag=null;
}
svg.addEventListener("pointerup",endPointer);
svg.addEventListener("pointercancel",endPointer);
gN.addEventListener("dblclick",ev=>{
  const nodeEl=ev.target.closest(".node");if(!nodeEl)return;
  const n=N.get(nodeEl.getAttribute("data-id"));
  n.pinned=false;n.g.classList.remove("pinned");reheat(0.3);
});
svg.addEventListener("wheel",ev=>{
  ev.preventDefault();
  zoomAbout((vb.w/W)*(ev.deltaY>0?1.12:0.89),ev.clientX,ev.clientY);
},{passive:false});

/* ============ legend / filters ============ */
const legend=$("#legend");
const groupsInData=[...new Set(CIRCLE.people.map(p=>p.group))];
const groupOrder=["core","aesthete","trials","society","family","chaeronea","later","beyond","liaisons"].filter(g=>groupsInData.includes(g));
...
const SAMPLES={"self-reported":'<line x1="1" y1="5" x2="33" y2="5" stroke="var(--edge)" stroke-width="2.9"/>',
  "second-hand":'<line x1="1" y1="5" x2="33" y2="5" stroke="var(--edge)" stroke-width="1.9" stroke-dasharray="7 5"/>',
  "uncorroborated":'<line x1="1" y1="5" x2="33" y2="5" stroke="var(--edge)" stroke-width="1.8" stroke-dasharray="1.5 6" stroke-linecap="round"/>',
  "married":'<line x1="1" y1="5" x2="33" y2="5" stroke="var(--edge)" stroke-width="5"/><line x1="1" y1="5" x2="33" y2="5" stroke="var(--paper)" stroke-width="1.8"/>',
  "attraction-expressed":'<line x1="1" y1="5" x2="27" y2="5" stroke="var(--edge)" stroke-width="1.7" stroke-dasharray="2 3 6 3"/><path d="M27,1.5 L33,5 L27,8.5 z" fill="var(--edge)"/>'};
function lineSample(c){
  return SAMPLES[c]?`<svg class="lsample" width="34" height="10" aria-hidden="true">${SAMPLES[c]}</svg>`:"";
}
const certOrder=["married","self-reported","second-hand","uncorroborated","attraction-expressed"].filter(c=>RELS.some(r=>r.certainty===c));
const ISOLATED=new Set(CIRCLE.people.map(p=>p.id).filter(id=>!(byPerson.get(id)||[]).length));
if(ISOLATED.size)certOrder.push(NO_EDGE);
const DEFAULT_OFF_GROUPS=new Set(["liaisons"]);
const offGroups=new Set(),offCerts=new Set();
...
const soloKey=Object.create(null), soloPrev=Object.create(null);
function registerAxis(name,keys,off,label,title){
  AXES[name]={keys,off,btns:new Map(),label,title};
}
function syncAxis(name){
  const a=AXES[name];if(!a)return;
  for(const [k,b] of a.btns){
    const on=soloKey[name]===k;
    b.setAttribute("aria-pressed",String(!a.off.has(k)));
    b.classList.toggle("solo",on);
    b.title=on?`Showing only ${a.label(k)} — click to bring the rest back`:a.title(k);
  }
}
function toggleSolo(name,key){
  const a=AXES[name];if(!a)return;
  if(soloKey[name]===key){
    a.off.clear();for(const k of soloPrev[name]||[])a.off.add(k);
    soloKey[name]=null;
  }else{
    soloPrev[name]=new Set(a.off);
    a.off.clear();for(const k of a.keys)if(k!==key)a.off.add(k);
    soloKey[name]=key;
  }
  syncAxis(name);applyFilters();
}
function lgBtn(name,key,html){
  const a=AXES[name];
  const b=document.createElement("button");
  b.className="lgc";b.innerHTML=html;
  a.btns.set(key,b);
  b.addEventListener("click",()=>{
    ...
    if(soloKey[name]===key){toggleSolo(name,key);return;}
    if(a.off.has(key))a.off.delete(key);else a.off.add(key);
    soloKey[name]=null;            // hand-editing a row ends its solo; the row keeps what it is now
    syncAxis(name);applyFilters();
  });
  b.addEventListener("contextmenu",ev=>{ev.preventDefault();toggleSolo(name,key);});
...
if(groupOrder.length){
  const t=document.createElement("span");t.className="lg-title";t.textContent="People";legend.appendChild(t);
  registerAxis("group",groupOrder,offGroups,g=>GROUP_LABEL[g]||g,
    g=>(DEFAULT_OFF_GROUPS.has(g)?`Show ${GROUP_LABEL[g]||g} (hidden by default)`
                                 :`Toggle ${GROUP_LABEL[g]||g}`)+SOLO_HINT);
  for(const g of groupOrder){
    if(DEFAULT_OFF_GROUPS.has(g))offGroups.add(g);
    lgBtn("group",g,`<span class="dot" style="background:var(--g-${g})"></span>${esc(GROUP_LABEL[g]||g)}`);
  }
  syncAxis("group");
  const sep=document.createElement("span");sep.className="lg-sep";legend.appendChild(sep);
}
const offGenders=new Set();
{
  const t=document.createElement("span");t.className="lg-title";t.textContent="Gender";legend.appendChild(t);
  const GLABEL={m:"men",f:"women"};
  registerAxis("gender",["m","f"],offGenders,k=>GLABEL[k],k=>`Toggle ${GLABEL[k]}`+SOLO_HINT);
  lgBtn("gender","m",`<svg width="12" height="12" aria-hidden="true"><rect x="1.5" y="1.5" width="9" height="9" rx="1" fill="none" stroke="var(--ink2)" stroke-width="1.5"/></svg>Men`);
  lgBtn("gender","f",`<svg width="12" height="12" aria-hidden="true"><circle cx="6" cy="6" r="4.5" fill="none" stroke="var(--ink2)" stroke-width="1.5"/></svg>Women`);
  syncAxis("gender");
  const br=document.createElement("span");br.className="lg-break";legend.appendChild(br);
}
if(certOrder.length){
  const t=document.createElement("span");t.className="lg-title";t.textContent="Connections";legend.appendChild(t);
  ...
  registerAxis("cert",certOrder,offCerts,c=>CERT_LABEL[c],
    c=>(DEFAULT_OFF.has(c)?`Show ${CERT_LABEL[c]} connections (hidden by default)`
...
let searchTerm="", searchScope="names";
...
const NAME_TEXT=new Map(), QUOTE_TEXT=new Map(), NOTE_TEXT=new Map();
{
  const push=(m,id,s)=>{if(s)m.set(id,(m.get(id)||"")+" "+String(s).toLowerCase());};
  for(const p of CIRCLE.people){
    push(NAME_TEXT,p.id,`${p.name} ${(p.aka||[]).join(" ")} ${p.sort_name||""}`);
    push(QUOTE_TEXT,p.id,p.bio);
    push(NOTE_TEXT,p.id,[p.bio_note,p.roster_note,
      ...(p.context_engagements||[]).flatMap(ce=>[ce.partner_name,ce.note,ce.date_label,ce.period])
    ].filter(Boolean).join("  "));
  }
  for(const r of RELS){
    ...
    for(const q of r.sources||[])bits.push(q.quote,q.translation,q.context,q.supports);
    const blob=bits.filter(Boolean).join("  ");
    const d=r.disputed||{};
    const notes=[r.certainty_reasoning,r.date_label,r.sources_order_note,r.prior_class,
      d.claim,d.asserted_by,d.disputed_by,d.grounds,
      ...(r.null_findings||[]).map(n=>typeof n==="string"?n:JSON.stringify(n)),
      ...(r.sources||[]).flatMap(q=>[q.provenance,q.translation_note,q.locator,
        q.order_hint&&q.order_hint.why])
    ].filter(Boolean).join("  ");
    for(const id of r.people){push(QUOTE_TEXT,id,blob);push(NOTE_TEXT,id,notes);}
  }
}
const SCOPE_PLACEHOLDER={names:"Search a name…",quotes:"Search quotations…",
  all:"Search names, quotations and notes…"};
function hitKind(p){                           // "name" | "quote" | "note" | null
  if(!searchTerm)return null;
  if(searchScope!=="quotes"&&(NAME_TEXT.get(p.id)||"").includes(searchTerm))return "name";
  if(searchScope!=="names"&&(QUOTE_TEXT.get(p.id)||"").includes(searchTerm))return "quote";
  ...
  if(searchScope==="all"&&(NOTE_TEXT.get(p.id)||"").includes(searchTerm))return "note";
  return null;
}
function updateSearchNote(){
  ...
  for(const p of CIRCLE.people){const k=hitKind(p); if(k)by[k]++;}
  const n=by.name+by.quote+by.note;
  if(!n){el.textContent="no match";return;}
  const people=`${n} ${n===1?"person":"people"}`;
  const parts=[by.name&&`${by.name} by name`,by.quote&&`${by.quote} in quotations`,
    by.note&&`${by.note} in notes`].filter(Boolean);
  el.textContent=parts.length>1?`${people} — ${parts.join(", ")}`
    :(by.name?people:`${people}, matched ${by.quote?"in quotations":"in notes"}`);
}
...
function personHidden(p){                      // dimmed by a legend switch
...
function personMatches(p){                     // survives the search box
...
function applyFilters(){
  ...
  for(const n of nodes){
    ...
    const allHidden=es.length?es.every(r=>offCerts.has(r.certainty)):offCerts.has(NO_EDGE);
...
$("#search").addEventListener("input",ev=>{
  searchTerm=ev.target.value.trim().toLowerCase();applyFilters();updateSearchNote();});
$("#searchscope").addEventListener("change",ev=>{
  searchScope=ev.target.value;
  $("#search").placeholder=SCOPE_PLACEHOLDER[searchScope]||"Search…";
  applyFilters();updateSearchNote();});
$("#search").addEventListener("keydown",ev=>{
  if(ev.key==="Escape"){ev.target.value="";searchTerm="";applyFilters();updateSearchNote();}
  if(ev.key==="Enter"&&searchTerm){
    ...
    const pick=named.length===1?named[0]:(any.length===1?any[0]:null);
...
function certBadge(r){
  const SAMPLES={"self-reported":'<line x1="1" y1="5" x2="27" y2="5" stroke="var(--edge)" stroke-width="2.9"/>',
    "second-hand":'<line x1="1" y1="5" x2="27" y2="5" stroke="var(--edge)" stroke-width="1.9" stroke-dasharray="7 5"/>',
    "uncorroborated":'<line x1="1" y1="5" x2="27" y2="5" stroke="var(--edge)" stroke-width="1.8" stroke-dasharray="1.5 6" stroke-linecap="round"/>',
    "married":'<line x1="1" y1="5" x2="27" y2="5" stroke="var(--edge)" stroke-width="5"/><line x1="1" y1="5" x2="27" y2="5" stroke="var(--paper)" stroke-width="1.8"/>',
    "attraction-expressed":'<line x1="1" y1="5" x2="21" y2="5" stroke="var(--edge)" stroke-width="1.7" stroke-dasharray="2 3 6 3"/><path d="M21,1.5 L27,5 L21,8.5 z" fill="var(--edge)"/>'};
  const dir=r.certainty==="attraction-expressed"&&r.direction&&P.get(r.direction)
    ? ` — ${esc(P.get(r.direction).name.split(" ").slice(-1)[0])} to ${esc(P.get(r.people.find(x=>x!==r.direction)).name.split(" ").slice(-1)[0])}, ${esc(OUTCOME_LABEL[r.outcome]||r.outcome||"")}` : "";
...
function miniBadge(c){
  ...
  return `<svg class="mini" width="24" height="8" aria-hidden="true"><line x1="1" y1="4" x2="23" y2="4" stroke="var(--edge)" ${
    c==="self-reported"?'stroke-width="2.6"':
    c==="second-hand"?'stroke-width="1.6" stroke-dasharray="6 4"':
    c==="uncorroborated"?'stroke-width="1.6" stroke-dasharray="1.5 5" stroke-linecap="round"':'stroke-width="4"'}/></svg>`;
}
...
function byEvidenceDate(list,get){
  const g=get||(x=>x);
  const key=x=>{const s=g(x),d=(s&&s.evidence_date)||(s&&s.order_hint); return (d&&d.y)?[d.y,d.m||0,d.d||0]:null;};
  return list.map((s,i)=>[s,i]).sort((a,b)=>{
    const ka=key(a[0]),kb=key(b[0]);
    if(ka&&kb){for(let i=0;i<3;i++)if(ka[i]!==kb[i])return ka[i]-kb[i];return a[1]-b[1];}
    if(ka)return -1; if(kb)return 1; return a[1]-b[1];
  }).map(x=>x[0]);
}
...
const NAME_INDEX=(()=>{
  ...
  for(const p of CIRCLE.people){
    ...
    if(p.sort_name&&p.sort_name.includes(",")){          // "Doyle, Peter" -> "Peter Doyle"
    ...
    for(const k of keys){
      const n=norm(k);
      if(!n)continue;
      if(m.has(n)&&m.get(n)!==p.id){m.set(n,null);continue;}   // ambiguous: link neither
...
function shortWho(s){
  if(!s)return "";
  const main=String(s).split(",")[0].trim().replace(/\s*\(.*\)$/,"");
  const w=main.split(/\s+/).filter(x=>!/^(sir|mr|mrs|dr|lord|lady|the)\.?$/i.test(x));
...
const ABBREV=/\b(?:Mr|Mrs|Ms|Dr|St|Sir|Rev|Hon|Capt|Col|Lieut|Lt|Prof|Messrs|Jr|Sr|Esq|No|vol|pp?)\.$/;
const ABBREV_ANY=/\b(?:Mr|Mrs|Ms|Dr|St|Sir|Rev|Hon|Capt|Col|Lieut|Lt|Prof|Messrs|Jr|Sr|Esq|No|vol|pp?)\./g;
function splitQuestionTail(s){
  const re=/[.!?…"'\]]\s+/g;
  let m,best=null;
  while((m=re.exec(s))){
    const head=s.slice(0,m.index+1), tail=s.slice(m.index+m[0].length).trim();
...
function parseExchange(q){
  ...
  if(!raw)return null;
  const turns=[];
  if(/\b[A-Z][A-Z .'-]*:/.test(raw)){                 // explicitly labelled speakers
    const parts=raw.split(/\s*\b([A-Z][A-Z .'-]*[A-Z]):\s*/).filter(s=>s!=="");
    if(parts.length<2)return null;
    // an unlabelled preamble before the first name keeps its own row, unattributed
    let i=0;
    if(!/^[A-Z][A-Z .'-]*[A-Z]$/.test(parts[0])){turns.push({who:"",text:parts[0]});i=1;}
  ...
  const asks=shortWho(q.addressee)||"Q.", says=shortWho(q.speaker)||"A.";
  for(let i=0;i<chunks.length;i++){
    ...
    else if(!/[.!?…]\s+\S/.test(c.replace(ABBREV_ANY,"")))turns.push({who:asks,text:c});
    /* More than one sentence and no boundary we trust: the turn changes somewhere inside this
       ...
       exists to prevent - committing it one line lower would be no better. */
...
const LANG_NAME={fr:"French",it:"Italian",de:"German",es:"Spanish",la:"Latin",el:"Greek",
  ru:"Russian",da:"Danish",nl:"Dutch",pt:"Portuguese"};
function personLink(name){
  const id=NAME_INDEX.get(name);
  return id?`<a class="qlink" href="#/p/${esc(id)}">${esc(name)}</a>`:esc(name);
}
...
function withBar(withWhom,when){
  ...
  return `<div class="qwith">${withWhom?`with <a href="#/r/${esc(withWhom.r.id)}">${esc(withWhom.other.name)}</a>${miniBadge(withWhom.r.certainty)}`:""}${when?`<span class="qdate">${esc(when)}</span>`:""}</div>`;
}
function quoteCard(q,withWhom,si){
  const [vLabel,vClass]=VER_META[q.verification||"unverified"]||VER_META.unverified;
  ...
  const byline=wk?(q.speaker||(q.voice==="modern"&&wk.editors?`${wk.editors}, eds.`:(wk.author||""))):"";
  ...
  const bylineHtml=byline?personLink(bcut>0?byline.slice(0,bcut):byline)+(bcut>0?esc(byline.slice(bcut)):""):"";
  const attr=wk?`— ${bylineHtml}${byline?inVol:""}<i>${esc(wk.title||q.work)}</i>${wk.year?` (${wk.year})`:""}${q.locator?`, ${esc(q.locator)}`:""}.`
             :(q.locator?`— ${esc(q.locator)}.`:"");
  ...
  const spk=q.voice==="period"?(q.speaker||(authIsEditor?"":auth)||""):"";
  const cut=spk.indexOf(",");
  let spkMain=cut>0?spk.slice(0,cut):spk, spkRest=cut>0?spk.slice(cut+1).trim():"";
  ...
  if(to&&/^(letter|note|telegram|postcard|diary|memoir)?\s*to\s+/i.test(spkRest))spkRest="";
  return `<div class="qcard"${siAttr}>
    ...
    ${(()=>{
      ...
      if(q.voice==="exchange"){
        ...
        if(turns)return `<blockquote class="q-exchange"${langAttr}>${turns.map(t=>
          `<span class="who">${esc(t.who||"")}</span><span class="said">${esc(t.text)}</span>`).join("")}</blockquote>`;
...
/* FILTERING A LONG RUN OF SOURCES. Wilde's page carries 178 quotations and the longest connection
   ...
   The bar is drawn only from SRC_FILTER_MIN sources up. Forty-one of the 106 connections here rest
   on a single quotation and fifty-eight on three or fewer; a filter above one card is furniture
   pretending to be a tool, and the cost of drawing it everywhere is paid on exactly the pages that
   need it least. */
const SRC_FILTER_MIN=4;
...
function srcYear(q){
  const d=(q&&q.evidence_date&&q.evidence_date.y!=null)?q.evidence_date
         :(q&&q.order_hint&&q.order_hint.y!=null)?q.order_hint:null;
  return d?d.y:null;
}
...
function srcHay(q,withWhom){
  const wk=CIRCLE.works[q.work]||null;
  return [q.quote,q.translation,q.translation_note,q.context,q.speaker,q.addressee,q.supports,
    q.locator,q.provenance,q.verified_by,
    wk&&wk.title,wk&&wk.author,wk&&wk.editors,wk&&wk.year,
...
function sourceFilterBar(n){
  ...
  const lo=ys.length?Math.min(...ys):"",hi=ys.length?Math.max(...ys):"";
...
function markTerm(root,term){
  ...
  const walk=document.createTreeWalker(root,NodeFilter.SHOW_TEXT,{acceptNode:n=>
    MARK_STOP.has(n.parentNode.nodeName)?NodeFilter.FILTER_REJECT
  ...
  for(const node of hits){
    ...
    while((j=s.toLowerCase().indexOf(low,i))>=0){
      ...
      m.className="sfhit";m.textContent=s.slice(j,j+low.length);
      frag.appendChild(m);n++;i=j+low.length;
    }
...
function flagHiddenHits(card){
  ...
  if(det&&!det.open&&det.querySelector("mark.sfhit"))det.classList.add("sfdeep");
}
...
function wireSourceFilter(){
  const box=$("#sfq");if(!box)return;
  const y1=$("#sfy1"),y2=$("#sfy2"),cnt=$("#sfcount"),clr=$("#sfclear"),none=$("#sfnone");
  const cards=[...panel.querySelectorAll(".qcard[data-si]")];
  const apply=()=>{
    const term=box.value.trim().toLowerCase();
    const a=parseInt(y1.value,10),b=parseInt(y2.value,10);
    let lo=Number.isFinite(a)?a:null,hi=Number.isFinite(b)?b:null;
    ...
    for(const el of cards){
      ...
      clearMarks(el);
      el.querySelectorAll("details.sfdeep").forEach(d=>d.classList.remove("sfdeep"));
      if(ok&&term.length>=MARK_MIN){markTerm(el,term);flagHiddenHits(el);}
    }
    const active=!!term||dated;
    cnt.textContent=active?`${shown} of ${cards.length}${dropped?` · ${dropped} undated hidden`:""}`:"";
  ...
  y1.addEventListener("input",apply);y2.addEventListener("input",apply);
  clr.addEventListener("click",()=>{y1.value="";y2.value="";apply();y1.focus();});
  ...
  for(const el of [y1,y2])el.addEventListener("keydown",e=>{
    if(e.key==="Escape"&&(y1.value||y2.value)){e.stopPropagation();y1.value="";y2.value="";apply();}});
}
...
function sourcesByPerson(id){
  ...
  const rels=(byPerson.get(id)||[]).filter(r=>(r.sources||[]).length);
  ...
  PANEL_SRC=[];
  const cards=byEvidenceDate(flat,f=>f.q).map(f=>{
    PANEL_SRC.push({hay:srcHay(f.q,f),y:srcYear(f.q)});
    return quoteCard(f.q,f,PANEL_SRC.length-1);
  }).join("");
  return `<div class="sect">Sources, in date order (${flat.length})</div>`
    +`<p class="sect-note">Every quotation from all ${rels.length} of this person's connections, in
       the order the evidence was made. Each says which connection it belongs to.</p>`
    +sourceFilterBar(flat.length)+cards;
}
function openPerson(id){
  const p=P.get(id);if(!p){closePanel();return;}
  const rels=(byPerson.get(id)||[]);
  const rows=[];
  if(p.born)rows.push({k:sortKey(p.born),cls:"anchor",when:fmtDate(p.born),what:"Born"});
  for(const r of rels){
    const q=P.get(partnerOf(r,id));
    rows.push({k:sortKey(r.start),cls:"",when:relDateLabel(r),
      what:`<a href="#/r/${esc(r.id)}">${esc(q.name)}</a>${miniBadge(r.certainty)}`});
  }
  for(const ce of p.context_engagements||[]){
    rows.push({k:sortKey(ce.start),cls:"ctx",
      when:ce.start||ce.end?`${fmtDate(ce.start)}${ce.end&&ce.end.y!=null?` – ${fmtDate(ce.end)}`:""}`:"date unknown",
  ...
  if(p.died)rows.push({k:sortKey(p.died),cls:"anchor",when:fmtDate(p.died),what:"Died"});
  const life=`${yearOf(p.born)}–${yearOf(p.died)}`;
  panel.innerHTML=`<div class="pwrap"><div class="grab"></div>
    <button class="pclose" onclick="location.hash=''">Close</button>
    ...
    ${rows.length?`<ul class="tl">${rows.map(r=>`<li class="${r.cls}"><span class="knot"></span><div class="when">${esc(r.when)}</div><div class="what">${r.what}</div></li>`).join("")}</ul>`
  ...
  wireSourceFilter();
  showPanel();focusGraph({person:id});
}
function openRel(id){
  const r=RELS.find(x=>x.id===id);if(!r){closePanel();return;}
  const [a,b]=r.people.map(pid=>P.get(pid));
  const d=r.disputed;
  PANEL_SRC=[];
  const srcCards=byEvidenceDate(r.sources||[]).map(q=>{
  ...
  wireSourceFilter();
  showPanel();focusGraph({rel:id});
}
function openList(){
  const sorted=[...RELS].sort((x,y)=>sortKey(x.start)-sortKey(y.start));
  panel.innerHTML=`<div class="pwrap"><div class="grab"></div>
    ${isDesktop()?"":`<button class="pclose" onclick="location.hash=''">Close</button>`}
    <div class="sect" style="margin-top:6px">All relationships · chronological (${sorted.length})</div>
    ${sorted.map(r=>{
      const [a,b]=r.people.map(pid=>P.get(pid));
      let ok=0,pend=0;for(const q of r.sources||[])(q.verification||"").startsWith("verified")?ok++:pend++;
  ...
  showPanel();focusGraph({});
}
function showPanel(){panel.classList.add("open");panel.scrollTop=0;}
...
function mountAbout(){
  ...
  host.innerHTML=CIRCLE.about.replace(/\{\{line:([a-z-]+)\}\}/g,(m,c)=>lineSample(c));
}
const isDesktop=()=>matchMedia("(min-width:901px)").matches;
function renderAbout(){                    // the sidebar's resting state on desktop
  panel.innerHTML=`<div class="pwrap"><div class="grab"></div>${$("#aboutBody").innerHTML}</div>`;
  showPanel();
}
...
function syncAbout(){
  const on=isDesktop()?!location.hash:$("#about").classList.contains("open");
  $("#btnAbout").setAttribute("aria-pressed",String(on));
}
function closePanel(){
  focusGraph({});
...
function focusGraph({person,rel}){
  ...
  if(person){
    const n=N.get(person);if(n)n.g.classList.add("focus","primary");
    for(const e of edges)if(e.r.people.includes(person)){e.g.classList.add("focus");N.get(partnerOf(e.r,person)).g.classList.add("focus");}
    bringIntoView(N.get(person)?[N.get(person)]:[]);
  }
  if(rel){
    const e=edges.find(x=>x.r.id===rel);
    if(e){e.g.classList.add("focus");e.a.g.classList.add("focus");e.b.g.classList.add("focus");
...
function bringIntoView(ns){
  ...
  if(x0>=vb.x+m&&x1<=vb.x+vb.w-m&&y0>=vb.y+m&&y1<=vb.y+vb.h-m)return;
  const aspect=paneAspect();
  let hw=Math.max(vb.w/2,(x1-x0)/2+m,((y1-y0)/2+m)*aspect);
  hw=Math.min(hw,W/2*ZOOM_OUT_MAX);
  const hh=hw/aspect;
  vb.w=hw*2; vb.h=hh*2; vb.x=(x0+x1)/2-hw; vb.y=(y0+y1)/2-hh;
  applyVB();
}
function route(){
...
$("#btnList").addEventListener("click",()=>{location.hash=location.hash==="#/list"?"":"#/list";});

/* ============ about / theme / stamp ============ */
$("#btnAbout").addEventListener("click",()=>{
  if(!isDesktop()){$("#about").classList.add("open");syncAbout();return;}
  if(location.hash)location.hash="";       // routes to closePanel -> renderAbout
  else renderAbout();
});
addEventListener("resize",()=>{if(!location.hash)closePanel();syncAbout();});
$("#aboutClose").addEventListener("click",()=>{$("#about").classList.remove("open");syncAbout();});
$("#about").addEventListener("click",ev=>{if(ev.target.id==="about"){$("#about").classList.remove("open");syncAbout();}});
addEventListener("keydown",ev=>{
  if(ev.key==="Escape"){
    if($("#about").classList.contains("open"))$("#about").classList.remove("open");
    else if(panel.classList.contains("open"))location.hash="";
  }
});
const vc=verCounts();
mountAbout();     // must precede the colophon fill below: it replaces the element that holds it
$("#stamp").textContent=CIRCLE.built==="unbuilt"?"":`${CIRCLE.people.length} people · ${RELS.length} connections · ✓ ${vc.ok} quotes verified, ⧖ ${vc.pend} quotes pending verification · updated ${CIRCLE.built}`;
$("#colophon").innerHTML=`Updated <b>${esc(CIRCLE.built)}</b> · ${CIRCLE.people.length} people, ${RELS.length} relationships, ${vc.total} source records (${vc.ok} verified, ${vc.pend} pending verification, ${vc.pointers} pointers)`;
...
function rescueOnLines(){
  let onLines=countOnLines(), overlaps=countOverlaps();
  if(!onLines)return;
  for(const n of nodes){
    ...
    const rest=(edges.find(e=>(e.a===n&&e.b===anchor)||(e.b===n&&e.a===anchor))||{}).rest||SIM.REST;
    let best=null;
    for(const mul of [1.0,1.3,1.6,1.9,2.2,2.6,3.0]){
      const rad=rest*mul;
      for(let k=0;k<48;k++){
        const ang=k*Math.PI*2/48;
        const x=anchor.x+Math.cos(ang)*rad, y=anchor.y+Math.sin(ang)*rad;
        n.x=x;n.y=y;                  // every ring position is fair game; there is no frame to stay inside
        const lc=nodeLineClearance(n);
        if(lc<n.r+CLEAR)continue;
        let gap=Infinity;
        for(const m of nodes){if(m===n)continue;gap=Math.min(gap,Math.hypot(m.x-n.x,m.y-n.y)-m.r-n.r);}
        if(gap<10)continue;
    ...
    const nowLines=countOnLines(), nowOver=countOverlaps();
    if(nowLines<onLines&&nowOver<=overlaps){onLines=nowLines;overlaps=nowOver;}
    else{n.x=home.x;n.y=home.y;}
  }
}
...
function countCrossings(){
  let c=0;
  const side=(ax,ay,bx,by,cx,cy)=>(cy-ay)*(bx-ax)-(by-ay)*(cx-ax);
  for(let i=0;i<edges.length;i++)for(let j=i+1;j<edges.length;j++){
    const e=edges[i],f=edges[j];
    if(e.a===f.a||e.a===f.b||e.b===f.a||e.b===f.b)continue;   // sharing a person is not a crossing
    const d1=side(f.a.x,f.a.y,f.b.x,f.b.y,e.a.x,e.a.y), d2=side(f.a.x,f.a.y,f.b.x,f.b.y,e.b.x,e.b.y),
          d3=side(e.a.x,e.a.y,e.b.x,e.b.y,f.a.x,f.a.y), d4=side(e.a.x,e.a.y,e.b.x,e.b.y,f.b.x,f.b.y);
...
function drawnLen(e){
  const dx=e.b.x-e.a.x, dy=e.b.y-e.a.y, L=Math.hypot(dx,dy)||1, ux=dx/L, uy=dy/L;
  const arrow=e.r.certainty==="attraction-expressed"&&e.r.direction;
  const src=(arrow&&e.r.direction===e.b.id)?e.b:e.a, tgt=(src===e.a)?e.b:e.a;
  const sx=(src===e.a)?ux:-ux, sy=(src===e.a)?uy:-uy;
  return L-trimPastLabel(src,sx,sy,src.r)-trimPastLabel(tgt,-sx,-sy,tgt.r+(arrow?11:0));
}
...
function spreadShortEdges(){
  ...
  for(let iter=0;iter<80;iter++){
    ...
    for(const e of edges){
      ...
      const ux=dx/L, uy=dy/L, step=Math.min(need,5);
      const da=degree(e.a.id)+1, db=degree(e.b.id)+1;
      let wa=e.a.pinned?0:db/(da+db), wb=e.b.pinned?0:da/(da+db);
      const tot=wa+wb; if(!tot)continue;
      wa/=tot; wb/=tot;
      e.a.x-=ux*step*wa; e.a.y-=uy*step*wa;
      e.b.x+=ux*step*wb; e.b.y+=uy*step*wb;
    }
    if(worst<=0.5)break;
    for(let i=0;i<nodes.length;i++)for(let j=i+1;j<nodes.length;j++){
      const a=nodes[i],b=nodes[j];
  ...
  if(now.c>was.c||now.o>was.o||now.l>was.l)snap.forEach(([x,y],i)=>{nodes[i].x=x;nodes[i].y=y;});
}
function treeLayoutSatellites(){
  ...
  let best={cross:countCrossings(),lines:countOnLines(),over:countOverlaps()};
  for(const [,ms] of members){
    ...
    for(const e of edges){
      ...
      adj.get(e.a.id).push(e.b.id); adj.get(e.b.id).push(e.a.id); ecount++;
    }
    if(ecount!==ms.length-1)continue;              // has a cycle: not a tree, leave it to the solver
    const root=ms.reduce((a,b)=>adj.get(b.id).length>adj.get(a.id).length?b:a,ms[0]);
    // depth and parents
    const par=new Map([[root.id,null]]), depth=new Map([[root.id,0]]), order=[root.id];
    for(let i=0;i<order.length;i++){
      const u=order[i];
      for(const v of adj.get(u))if(!depth.has(v)){par.set(v,u);depth.set(v,depth.get(u)+1);order.push(v);}
    }
    ...
    for(let i=order.length-1;i>=0;i--){
      const id=order[i], ks=kids.get(id);
      leaves.set(id,ks.length?ks.reduce((s,k)=>s+leaves.get(k),0):1);
    }
    const maxDepth=Math.max(...depth.values())||1;
    const cx0=ms.reduce((s,n)=>s+n.x,0)/ms.length, cy0=ms.reduce((s,n)=>s+n.y,0)/ms.length;
    const spread=Math.max(...ms.map(n=>Math.hypot(n.x-cx0,n.y-cy0)));
    const ring=Math.max(105,Math.min(165,spread/maxDepth));
    const home=ms.map(n=>({n,x:n.x,y:n.y}));
    let bestRot=null;
    for(let r=0;r<16;r++){
      const rot=r*Math.PI*2/16, ang=new Map();
      const place=(id,a0,a1)=>{
        const mid=(a0+a1)/2;
        ang.set(id,mid);
        const N0=N.get(id), d=depth.get(id);
      ...
      N.get(root.id).x=cx0; N.get(root.id).y=cy0;
      const cross=countCrossings(), lines=countOnLines(), over=countOverlaps();
      if(!bestRot||cross<bestRot.cross||(cross===bestRot.cross&&over<bestRot.over))
        bestRot={rot,cross,lines,over,pos:ms.map(n=>[n.x,n.y])};
    }
    const ok=bestRot&&bestRot.cross<=best.cross&&bestRot.over<=best.over&&bestRot.lines<=best.lines;
    if(ok){
      bestRot.pos.forEach(([x,y],i)=>{ms[i].x=x;ms[i].y=y;});
      best={cross:bestRot.cross,lines:bestRot.lines,over:bestRot.over};
    }else{
      home.forEach(h=>{h.n.x=h.x;h.n.y=h.y;});
    }
  }
}
...
function packComponents(){
  const key=(i,j)=>i+","+j;
  const cellOf=(x,y)=>[Math.floor(x/PACK_CELL),Math.floor(y/PACK_CELL)];
  const members=new Map(), cedges=new Map();
  for(const n of nodes){
    const ci=compOf.get(n.id);
    if(!members.has(ci))members.set(ci,[]);
    members.get(ci).push(n);
  ...
  const cellsOf=(ci)=>{
    ...
    for(const n of members.get(ci)||[]){
      const [i0,j0]=cellOf(n.x-n.hw,n.y-n.r), [i1,j1]=cellOf(n.x+n.hw,n.y+n.r+LABEL_BELOW+3);
    ...
    for(const e of cedges.get(ci)||[]){
      ...
      for(let t=0;t<=steps;t++){
        const [i,j]=cellOf(e.a.x+dx*t/steps, e.a.y+dy*t/steps);
        s.add(key(i,j));
      }
    }
    return s;
  };
  const dilate=(s)=>{                       // one cell of breathing room, so nothing sits flush
    const out=new Set();
    for(const k of s){
      const [i,j]=k.split(",").map(Number);
      for(let a=-1;a<=1;a++)for(let b=-1;b<=1;b++)out.add(key(i+a,j+b));
    }
    return out;
  };
  ...
  const cand=[];
  for(let i=-46;i<=46;i++)for(let j=-46;j<=46;j++)cand.push([hi+i,hj+j,i*i+j*j]);
  cand.sort((a,b)=>a[2]-b[2]);
  const sats=[...members.keys()].filter(ci=>ci!==mainComp)
  ...
  for(const ci of sats){
    ...
    const rel=[...own].map(k=>{const [i,j]=k.split(",").map(Number);return [i-oi,j-oj];});
    let placed=null;
    for(const [ai,aj] of cand){
      let fits=true;
      for(const [di,dj] of rel)if(occupied.has(key(ai+di,aj+dj))){fits=false;break;}
      if(fits){placed=[ai,aj];break;}
    }
    if(!placed)continue;                    // nowhere in range: leave it where the solver put it
    const dx=(placed[0]-oi)*PACK_CELL, dy=(placed[1]-oj)*PACK_CELL;
    for(const n of cs){n.x+=dx;n.y+=dy;}
  ...
  for(let k=0;k<maxIter;k++){
    ...
    for(const n of nodes){
      ...
      for(const e of edges){
        ...
        const x1=e.a.x,y1=e.a.y,dx=e.b.x-x1,dy=e.b.y-y1,L2=dx*dx+dy*dy||1;
        let t=((n.x-x1)*dx+(n.y-y1)*dy)/L2; t=Math.max(0,Math.min(1,t));
        const ox=n.x-(x1+t*dx),oy=n.y-(y1+t*dy),d=Math.hypot(ox,oy)||0.01;
        const min=n.r+CLEAR;
        if(d<min){ax+=ox/d*(min-d);ay+=oy/d*(min-d);}
      }
      if(!ax&&!ay)continue;
      ...
      const mag=Math.hypot(ax,ay); if(mag>worst)worst=mag;
      const step=Math.min(mag,3.5)/mag;
      n.x+=ax*step;n.y+=ay*step;      // no walls to bounce off any more; outward is always open
    }
    for(let i=0;i<nodes.length;i++)for(let j=i+1;j<nodes.length;j++){
      const a=nodes[i],b=nodes[j];
      boxPush(a,b,4,(axd,ayd,bxd,byd)=>{
...
function centreOnHub(){        // no longer called
  if(!nodes.length)return;
  const hub=nodes.reduce((a,b)=>degree(b.id)>degree(a.id)?b:a,nodes[0]);
  const dx=W/2-hub.x, dy=H/2-hub.y;
  ...
  let hw=Math.max(ex,ey*aspect), hh=hw/aspect;
  if(hh<ey){hh=ey;hw=hh*aspect;}
  vb.w=hw*2; vb.h=hh*2;
  vb.x=W/2-hw; vb.y=H/2-hh;
  applyVB();
}
...
function openOnHub(){
  if(!nodes.length)return;
  const hub=N.get("wilde")||nodes.reduce((a,b)=>degree(b.id)>degree(a.id)?b:a,nodes[0]);
  const hw=W*OPEN_SCALE/2, hh=hw/paneAspect();
  vb.w=hw*2; vb.h=hh*2; vb.x=hub.x-hw; vb.y=hub.y-hh;
  applyVB();
}
...
function fitAll(){
  ...
  for(const n of nodes){
    const p=24;
    x0=Math.min(x0,n.x-n.hw-p); x1=Math.max(x1,n.x+n.hw+p);
    y0=Math.min(y0,n.y-n.r-p);  y1=Math.max(y1,n.y+n.r+LABEL_BELOW+p);
  }
  const aspect=paneAspect();
  let hw=Math.max((x1-x0)/2,(y1-y0)/2*aspect), hh=hw/aspect;
  const cx=(x0+x1)/2, cy=(y0+y1)/2;
  vb.w=hw*2; vb.h=hh*2; vb.x=cx-hw; vb.y=cy-hh;
  applyVB();
}
openOnHub();
$("#btnFit").addEventListener("click",fitAll);
addEventListener("resize",()=>{     // the pane's aspect drives both frames; a resize invalidates them
  const hw=vb.w/2, hh=hw/paneAspect();
  vb.y+=vb.h/2-hh; vb.h=hh*2; applyVB();
});
if(reduceMotion){render();}
  ...
  const SETTLE_MS=620, cx=W/2, cy=H/2;
  ...
  const home=nodes.map((n,i)=>{
    ...
    const inward=0.022*r, ia=Math.atan2(dy,dx);
    ...
    const wa=hash(i*3.7)*Math.PI*2, wm=9+hash(i*11.3)*11;
    return {n,x:n.x,y:n.y,
            ox:-Math.cos(ia)*inward+Math.cos(wa)*wm,
            oy:-Math.sin(ia)*inward+Math.sin(wa)*wm,
            delay:hash(i*7.1)*0.36};
  });
  for(const h of home){h.n.x=h.x+h.ox; h.n.y=h.y+h.oy;}
  const t0=performance.now();
  (function settleIn(now){
    const u=(now-t0)/SETTLE_MS;
    for(const h of home){
      const t=Math.min(1,Math.max(0,(u-h.delay)/(1-h.delay))), e=1-Math.pow(1-t,3);
      h.n.x=h.x+h.ox*(1-e); h.n.y=h.y+h.oy*(1-e);
    }
    render();
...
window.circle={CIRCLE,nodes,edges,RELS,P,byPerson,applyFilters,byEvidenceDate,offGroups,offCerts,
  route,parseExchange};
})();
